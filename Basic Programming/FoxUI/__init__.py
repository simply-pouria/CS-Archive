...
class cprint:
    ...
    def conti(text):
        cprint(f"*️⃣  {text}", fg_preset='white', bg_preset='darkerblue', pad=True, end="")
        print("\t", end="")
        a = input()
        clear_screen()
        return a
    ...
    def answer(text, end="\n"):
        cprint(f"✅ {text} ", fg_preset='white', bg_preset='darkergreen', pad=True)
    def info(text, end="\n"):
        cprint(f"*️⃣  {text} ", fg_preset='white', bg_preset='darkerblue', pad=True)
    ...
    def list(lst):
        for i in lst:
            ...
            if i != "Exit":
                t,d = i.split("-")
                cprint(f"👉 {t}", fg_preset='white', bg_preset='darkblue', pad=True, end="")
                cprint(f"{d}", fg_preset='white', bg_preset='darkerblue', pad=True, end="\n")
            else:
                
                cprint(f"❌ 00", fg_preset='white', bg_preset='darkred', pad=True, end="")
                cprint(f"{i}", fg_preset='white', bg_preset='darkerred', pad=True, end="\n")